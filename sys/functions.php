...
function deleteExpiredTestUsersByOwner($owner_id, $remove_expired, $remove_test, $start_date, $end_date)
{
	...
	if ($PDO !== NULL) {
		$sql = '';
		if ($remove_expired && $remove_test) {
			$sql = 'DELETE FROM `users` WHERE (`member_id` = :owner_id AND `created_at` >= :start_date AND `created_at` <= :end_date) AND (unix_timestamp(NOW()) > `exp_date` OR `is_trial` = 1)';
		} else if ($remove_expired) {
			$sql = 'DELETE FROM `users` WHERE (`member_id` = :owner_id AND `created_at` >= :start_date AND `created_at` <= :end_date) AND (unix_timestamp(NOW()) > `exp_date`)';
		} else if ($remove_test) {
			$sql = 'DELETE FROM `users` WHERE (`member_id` = :owner_id AND `created_at` >= :start_date AND `created_at` <= :end_date) AND (`is_trial` = 1)';
		}
...
function insertTest($email)
{
	...
	if ($PDO !== NULL) {
		$sql = 'INSERT INTO `test_historic` (`email`) VALUES (:email)';
		$stmt = $PDO->prepare($sql);
		$stmt->bindParam(':email', $email, PDO::PARAM_STR, 255);
...
function createFastTest($owner_id, $package_id)
{
	...
	if ($package && $package['is_trial']) {
		...
		if (createClient($owner_id, $username, $password, $duration, $package['bouquets'], 'BlackBoxHD.', 1)) {
...
function insertClient($owner_id, $username, $password, $exp_date, $admin_notes, $reseller_notes, $bouquet, $max_connections, $is_trial)
{
	...
	if ($PDO !== NULL) {
		$sql = 'INSERT INTO `users` (`member_id`, `username`, `password`, `exp_date`, `admin_notes`, `reseller_notes`, `bouquet`, `max_connections`, `is_trial`, `created_at`, `created_by`) VALUES             (:owner_id, :username, :password, :exp_date, :admin_notes, :reseller_notes, :bouquet, :max_connections, :is_trial, unix_timestamp(NOW()), :owner_id);';
		$stmt = $PDO->prepare($sql);
		$stmt->bindParam(':owner_id', $owner_id, PDO::PARAM_INT);
		$stmt->bindParam(':username', $username, PDO::PARAM_STR, 255);
		$stmt->bindParam(':password', $password, PDO::PARAM_STR, 255);
		...
		if ($stmt->execute()) {
			...
			$sql = 'INSERT INTO `user_output` (`user_id`, `access_output_id`) VALUES (:userid1, \'1\'), (:userid2, \'2\')';
			$stmt = $PDO->prepare($sql);
...
function getAllClientsAdminWithOptions($start = 0, $length = 10, $columns = array(), $search_value = '', $order_column_index = NULL, $order_type = 'asc')
{
	...
	if ($order_column_index !== NULL) {
		if (isset($columns[$order_column_index]['db'])) {
	...
	if (isset($search_value) && !empty($search_value) && is_string($search_value)) {
		$all_clients = huahuahuahu($search_value, $all_clients);
	}
...
function injectOnlineClientsLocation($logged_user)
{
	$online_users_loc = (isAdmin($logged_user) ? getAllOnlineClientsLocation() : getAllOnlineClientsLocationByOwnerID($logged_user['id']));
	$result = '';

	foreach ($online_users_loc as $loc => $count) {
		$result .= $loc . ': ' . $count . ', ';
	}
...
function getAllOnlineClientsLocation()
{
	$PDO = getconnection();

	if ($PDO !== NULL) {
		$sql = 'SELECT DISTINCT `geoip_country_code`, COUNT(`geoip_country_code`) AS Count FROM `user_activity_now` WHERE `geoip_country_code` != \'\' GROUP BY `geoip_country_code`';
		$stmt = $PDO->prepare($sql);
		$stmt->execute();
...
function getAllOnlineClientsLocationByOwnerID($userid)
{
	$PDO = getconnection();

	if ($PDO !== NULL) {
		$sql = 'SELECT DISTINCT `user_activity_now`.`geoip_country_code`, COUNT(`user_activity_now`.`geoip_country_code`) AS Count FROM `user_activity_now`, `users` WHERE `user_activity_now`.`geoip_country_code` != \'\' AND `users`.`id` = `user_activity_now`.`user_id` AND `users`.`member_id` = :userid GROUP BY `user_activity_now`.`geoip_country_code`';
		$stmt = $PDO->prepare($sql);
		$stmt->bindParam(':userid', $userid, PDO::PARAM_INT);
		$stmt->execute();
		return $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
...
function insertReseller($owner_id, $username, $password, $credits, $email, $notes, $member_group_id, $language)
{
	...
	if ($PDO !== NULL) {
		$sql = 'INSERT INTO `reg_users` (`username`, `password`, `email`, `ip`, `date_registered`, `verify_key`, `last_login`, `member_group_id`, `verified`, `credits`, `notes`, `status`, `default_lang`, `reseller_dns`, `owner_id`, `override_packages`, `google_2fa_sec`) VALUES (:username, :password, :email, NULL, unix_timestamp(NOW()), NULL, NULL, :member_group_id, \'1\', :credits, :notes, \'1\', :language, \'\', :owner_id, NULL, \'\')';
		$stmt = $PDO->prepare($sql);
		$stmt->bindParam(':owner_id', $owner_id, PDO::PARAM_INT);
		$stmt->bindParam(':username', $username, PDO::PARAM_STR, 255);
		$stmt->bindParam(':password', $password, PDO::PARAM_STR, 255);
...
function getResellersAdmin()
{
	...
	foreach ($all_users as $current_user) {
		...
		$reseller_group = array_search($current_user["member_group_id"], array_column($groups, "group_id"));
		$group_name = $reseller_group !== false ? $groups[$reseller_group]["group_name"] : "-";
...
function getAllResellersAdminWithOptions($start = 0, $length = 10, $columns = array(), $search_value = '', $order_column_index = NULL, $order_type = 'asc')
{
	...
	if (isset($search_value) && !empty($search_value) && is_string($search_value)) {
		$all_resellers = huahuahuahu($search_value, $all_resellers);
	}
...
function getAllResellersByOwnerWithOptions($reseller, $start = 0, $length = 10, $columns = array(), $search_value = '', $order_column_index = NULL, $order_type = 'asc')
{
	...
	foreach ($users as $foreachBox) {
		$current_user = $foreachBox;
		$reseller_key = array_search($current_user['owner_id'], array_column($all_users, 'id'));
		$reseller_name = ($reseller_key !== false ? $all_users[$reseller_key]['username'] : '-');
		$current_user['reseller_name'] = $reseller_name;
	...
	if ($reseller) {
		$columns = array(
			...
			array('db' => 'display_username', 'formatter' => function ($d, $row) {
				return $row['is_trial'] ? '<i class="fas fa-user-clock" data-toggle="tooltip" data-original-title="Sou um Teste"></i> ' . $row['username'] : $row['username'];
			}),
			array('db' => 'password'),
			...
			array('db' => 'status', 'formatter' => function ($d, $row) {
				...
				if ($row['admin_enabled'] && $row['enabled']) {
					...
					if (!$row['exp_date'] || (time() < $row['exp_date'])) {
						$status = '<i class="text-success fas fa-circle"></i>';
					} else {
						$status = '<i class="text-warning fas fa-circle"></i>';
					}
				} else if ($row['admin_enabled']) {
					$status = '<i class="text-secondary fas fa-circle">';
				} else {
					$status = '<i class="text-danger fas fa-circle">';
				}
			...
			array('db' => 'action', 'searchable' => false, 'formatter' => function ($d, $row) {
				...
				<div class="actions text-center">
				    <button type="button" class="btn btn-icon text-cyan" data-toggle="tooltip" data-placement="left" title="" data-original-title="' . $row['reseller_notes'] . '"><i class="fas fa-sticky-note"></i></button>
				    ...
				    <a href="#" class="btn btn-icon text-light-blue btlink" data-toggle="tooltip" data-original-title="Gerar Link" data-id="' . $row['id'] . '" data-user="' . $row['username'] . '" data-pass="' . $row['password'] . '">
				    ...
				    <a href="#" class="btn btn-icon text-green btrenew" data-toggle="tooltip" data-original-title="Renovar 1 mes - custo ' . $row['max_connections'] . ' credito(s)." data-id="' . $row['id'] . '" data-text="Usuario: ' . $row['username'] . ' - Creditos a ser consumido: ' . $row['max_connections'] . '">
				        <i class="far fa-calendar-alt" aria-hidden="true" style="font-size: 16px"></i>
				    </a>
				    <a href="#" class="btn btn-icon text-purple btrenewplus" data-toggle="tooltip" data-original-title="Renovar vários meses - custo depende da quantidade de meses e telas." data-id="' . $row['id'] . '" data-text="Usuario: ' . $row['username'] . '">
				        <i class="far fa-calendar-plus" aria-hidden="true" style="font-size: 16px"></i>
				    </a>
				    <a href="#" class="btn btn-icon text-aqua bttela" data-toggle="tooltip" data-original-title="Aumentar 1 tela - custo 1 credito(s)." data-id="' . $row['id'] . '" data-text="Usuario: ' . $row['username'] . ' - Creditos a ser consumido: 1">
				        <i class="fas fa-desktop" aria-hidden="true" style="font-size: 16px"></i>
				    </a>
	...
	if ($reseller) {
		$columns = array(
			...
			array('db' => 'country', 'formatter' => function ($d, $row) {
				...
				return '<img align="center" src="https://www.ip-tracker.org/images/ip-flags/' . strtolower($country) . '.png"/>';
			})
		);
...
function getAllResellersTable($userid, $start, $length, $search, $order_column_index, $order_type)
{
	...
	if ($reseller) {
		$columns = array(
			...
			array('db' => 'reseller_name'),
			array('db' => 'status', 'formatter' => function ($d, $row) {
				return $row['status'] ? '<i class="text-success fas fa-circle"></i>' : '<i class="text-danger fas fa-circle"></i>';
			}),
			array('db' => 'action', 'searchable' => false, 'formatter' => function ($d, $row) {
				return '
				<div class="actions text-center">
				    <button type="button" class="btn btn-icon text-cyan" data-toggle="tooltip" data-placement="left" title="" data-original-title="' . $row['notes'] . '"><i class="fas fa-sticky-note"></i></button>
...
function getTickets($userid, $start, $length, $search, $order_column_index, $order_type)
{
	...
	if ($reseller) {
		$columns = array(
			...
			array('db' => 'action', 'searchable' => false, 'formatter' => function ($d, $row) {
				return '<div class="actions text-center"><a href="./ticket.php?ticket_id=' . $row['id'] . '" class="btn btn-icon text-muted" data-toggle="tooltip" data-original-title="Ver Ticket" data-id="' . $row['id'] . '"><i class="fa fa-search" aria-hidden="true" style="font-size: 16px"></i></a><a href="#" class="btn btn-icon text-yellow bttoggle" data-toggle="tooltip" data-original-title="Abrir/Fechar" data-id="' . $row['id'] . '" data-text="Abrir/Fechar o ticket"><i class="fa fa-ban" aria-hidden="true" style="font-size: 16px"></i></a></div>';
			})
		);
		$columns_admin = array(
			array('db' => 'id'),
			...
			array('db' => 'action', 'searchable' => false, 'formatter' => function ($d, $row) {
				return '<div class="actions text-center"><a href="./ticket.php?ticket_id=' . $row['id'] . '" class="btn btn-icon text-muted" data-toggle="tooltip" data-original-title="Ver Ticket" data-id="' . $row['id'] . '"><i class="fa fa-search" aria-hidden="true" style="font-size: 16px"></i></a><a href="#" class="btn btn-icon text-yellow bttoggle" data-toggle="tooltip" data-original-title="Abrir/Fechar" data-id="' . $row['id'] . '" data-text="Abrir/Fechar o ticket"><i class="fa fa-ban" aria-hidden="true" style="font-size: 16px"></i></a><a href="#" class="btn btn-icon text-red btdelete" data-toggle="tooltip" data-original-title="Deletar Ticket" data-id="' . $row['id'] . '" data-text="Deletar o ticket"><i class="fa fa-trash" aria-hidden="true" style="font-size: 16px"></i></a></div>';
			})
		);
...
function transferCredits($from_id, $to_id, $credits)
{
	...
	if ($from_reseller && $to_reseller) {
		...
		if (((0 <= $from_credits) || isAdmin($from_reseller)) && (0 <= $to_credits)) {
			$PDO = getconnection();
			...
			if ($PDO !== NULL) {
				...
				if (!isAdmin($from_reseller)) {
					$stmt->bindParam(':credits', $from_credits, PDO::PARAM_INT);
					$stmt->bindParam(':user_id', $from_reseller['id'], PDO::PARAM_INT);
				...
				$stmt->bindParam(':credits', $to_credits, PDO::PARAM_INT);
				$stmt->bindParam(':user_id', $to_reseller['id'], PDO::PARAM_INT);
...
function getLastChannels($limit = 10)
{
	...
	if ($PDO !== NULL) {
		$sql = 'SELECT `stream_display_name`, `added`, `category_id` FROM `streams` WHERE `type` = 1 OR `type` = 3 ORDER BY `added` DESC LIMIT :_limit;';
		$stmt = $PDO->prepare($sql);
...
function getLastMovies($limit = 10)
{
	...
	if ($PDO !== NULL) {
		$sql = 'SELECT `stream_display_name`, `added`, `category_id` FROM `streams` WHERE `type` = 2 ORDER BY `added` DESC LIMIT :_limit;';
		$stmt = $PDO->prepare($sql);
		$stmt->bindParam(':_limit', $limit, PDO::PARAM_INT);
...
function getLastSeries($limit = 10)
{
	...
	if ($PDO !== NULL) {
		$sql = 'SELECT `title`, `last_modified`, `category_id` FROM `series` ORDER BY `id` DESC LIMIT :_limit;';
		$stmt = $PDO->prepare($sql);
		$stmt->bindParam(':_limit', $limit, PDO::PARAM_INT);
...
function isMaster($user)
{
	...
	if ($group_settings['master'] == $user['member_group_id']) {
		...
		if ($owner_id) {
			...
			if ($owner) {
				if ($group_settings['master'] == $owner['member_group_id']) {
					return false;
				}
			}
		}
...
function createTicket($logged_user, $reseller_id, $title, $message)
{
	...
	if ($ticket_id !== false) {
		if (insertTicketReply($ticket_id, $admin_read, $message)) {
			return $ticket_id;
		}
	}
...
function insertRegUserLog($owner, $username, $password, $type)
{
	...
	if ($PDO !== NULL) {
		$sql = 'INSERT INTO `reg_userlog` (`owner`, `username`, `password`, `date`, `type`) VALUES (:owner, :username, :password, unix_timestamp(NOW()), :type);';
		$stmt = $PDO->prepare($sql);
...
function injectBtnColor()
{
	echo 'bg-' . str_replace(array('skin-', '-light'), '', getserverproperty('theme_color', 'skin-red'));
}

function getCreditsByUser($user)
{
	return isadmin($user) ? '&#8734;' : $user['credits'];
}
...
function GetList($username, $password)
{
	...
	$list_url = $server_dns . '/get.php?username=' . $username . '&password=' . $password . '&type=m3u_plus&output=mpegts';
	return $list_url;
}
...
function checkLicence($licence_key)
{
	...
	if (file_exists($licence_path)) {
		if (!@include_once($licence_path)) {
	...
	$check_token = time() . md5(mt_rand(1000000000, 9999999999) . $licence_key);
	$checkdate = date('Ymd');
	...
	unset($secret_key, $check_token, $checkdate, $cinewpurl, $post_query, $data, $response_code, $data_encoded);
	return $result;
}
...
define('KOFFICE_PANEL_VERSION', 'v2.1');
...
function ClearTestHist()
{
	$PDO = getofficeconnection();

	if ($PDO !== NULL) {
		$sql = 'DELETE FROM `test_historic` WHERE `id` > 0';
		$stmt = $PDO->prepare($sql);
...
function huahuahuahu($huqh, $lsalh)
{
	if (!is_array($lsalh)) {
		return [];
	}

	$huqh = mb_strtolower($huqh);

	return array_filter($lsalh, function ($item) use ($huqh) {
		$data = [];

		foreach ($item as $value) {
			$data[] = mb_strtolower($value);
		}

		if (hauhuahauhu(array_values($data), $huqh) !== false) {
			return true;
		}
...
function hauhuahauhu($shushuhushu, $nejrkferkf, $offset = 0)
{
	if (!is_array($shushuhushu)) $shushuhushu = array($nejrkferkf);

	$ahuhuahu = [];

	foreach ($shushuhushu as $hushuhushu) {
		if (mb_strpos($hushuhushu, $nejrkferkf) !== false) {
			$ahuhuahu[] = true;
		}
	}

	if (!empty($ahuhuahu)) return true;
